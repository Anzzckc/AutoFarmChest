...
task.spawn(function()
    pcall(function()
        local pgui = LocalPlayer:WaitForChild("PlayerGui", 60)
        local main = pgui:WaitForChild("Main", 60)
        local choose = main:WaitForChild("ChooseTeam", 60)
        for _ = 1, 300 do
            if choose.Visible or LocalPlayer.Team ~= nil then break end
            task.wait(0.1)
        end
        if choose.Visible then
            local btn = choose.Container.Marines.Frame.ViewportFrame.TextButton
